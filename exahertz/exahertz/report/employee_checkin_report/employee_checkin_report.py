...
def get_data(filters):
	...
	for row in data:
		...
		if row.get('last_out') and row.get('shift_end'):
			...
			row['early_exit_by'] = "No Checkout"

		# Compute final status
		if not row.get('attendance_status'):
			if row.get('first_in') and row.get('last_out'):
				row['attendance_status'] = 'OK'
			elif row.get('first_in') and not row.get('last_out'):
				row['attendance_status'] = 'No Checkout'
			elif not row.get('first_in') and row.get('last_out'):
				row['attendance_status'] = 'No Check-in'
			else:
