...
def on_employee_checkin_submit(doc, method=None):
	...
	The scheduler job handles all regularization creation because:
	...
	if checkin_date < sync_date:
		...
		frappe.enqueue(
			'exahertz.exahertz.overrides.employee_checkin.consolidate_attendance_for_date',
			queue='short',
			timeout=300,
	...
	frappe.logger().debug(
		f"Checkin {doc.name} recorded. Regularization will be processed by scheduler."
	)
...
def run_attendance_regularization_sync(days: int = 365, include_yesterday: bool = True):
	...
	frappe.enqueue(
		'exahertz.exahertz.overrides.employee_checkin.process_attendance_sync_background',
		queue='long',
		timeout=3600,
		days=days,
	...
	for sa in active_employees:
		...
		if not checks:
			try:
				...
				# Check for approved Attendance Request before marking Absent
		...
		from exahertz.exahertz.overrides.attendance_utils import get_first_in_last_out
		first_in, last_out = get_first_in_last_out(checks, use_inferred=True)

		from exahertz.exahertz.overrides.attendance_utils import calculate_late_early_times
