...
function test_individual_connection(frm, config_row) {
    ...
    frappe.call({
        method: 'exahertz.exahertz.doctype.crosschex_settings.crosschex_settings.test_individual_api_config',
        args: {
        ...
        callback: function(r) {
            if (r.message && r.message.success) {
                ...
                // Update parent connection status
                frm.set_value('connection_status', 'Error');
                frm.refresh_field('api_configurations');
                frm.save();
            }
        }
...
function sync_individual_device(frm, config_row) {
    ...
    frappe.call({
        method: 'exahertz.exahertz.doctype.crosschex_settings.crosschex_settings.sync_individual_device',
        args: {
            api_url: config_row.api_url,
            api_key: config_row.api_key,
            config_row_name: config_row.name,
